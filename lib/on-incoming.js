 ...
 * @private
 */
const debug = require('debug')('ccr:on-incoming')
...
function onRequest (connection, worker, message) {
  ...
  let done = err => {
    ...
    // Exit now if request is sent
    if (req._sent) {
      debug('Can only respond when a response (for request %s) is not already sent', req.key)
  ...
  // attach final handler to request
  req.next = done
...
function onResponse (connection, message) {
  ...
  // Only if we have a callback do we proceed.
    ...
    debug('Opening response for %s on Channel %s', key, connection.channel)
