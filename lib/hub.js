...
class ConnectionHub extends Base {
  ...
  set (key, value, overwrite) {
    ...
    if (key === 'routes') {
      ...
      return this
    // Set all keys to lower case in insensitive mode.
    } else if (keys[0] === 'routes') {
      if (keys.length === 2) {
        let type = typeof value
        if (type === 'object') {

        } else {
          this._keys[keys[1]] = undefined
        }
      } else if (keys.length === 3 && keys[2] === 'route') {
    ...
    // Abort on any match of keys in `BANNED`.
    if (includes(BANNED, keys[0])) return this
  ...
  start () {
    ...
    this._router.use(':key/*', checkForKey(this), sendToDefault(this))
...
function setKeys (connection, value) {
  ...
  if (type === 'string') {
    // Get paths
    let paths = fs.readdirSync(path.resolve(connection.get('module root'), value))
    ...
    paths.forEach(string => {
      ...
      if (path.extname(string) !== '.js') return
      // Push key (gotten from filename)
      keys.push(path.basename(string, '.js'))
    })
  ...
  keys.forEach(key => !includes(RESERVED, key) ? (connection._keys[key] = undefined) : false)
}
