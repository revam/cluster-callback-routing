...
module.exports = class Router {
  ...
  route (path) {
    ...
    if (callbacks.length === 0) {
      throw new TypeError('Router.route() requires functions')
    }
    ...
    for (let i = 0; i < callbacks.length; i++) {
      ...
      if (typeof fn !== 'object' && fn.handle) {
        ...
        throw new TypeError('Router.route() requires a function but got a ' + getType(fn))
      }

      // add the middleware
      debug('route %s %s', path, fn.name || '<anonymous>')
